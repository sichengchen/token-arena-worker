...
export async function generateMetadata({ params }: PublicProfilePageProps): Promise<Metadata> {
  ...
  const pageUrl = buildAbsoluteUrl(`/${locale}/u/${encodeURIComponent(username)}`);
          ...
          <Card className="bg-card shadow-sm ring-1 ring-border/60">
            <CardContent className="space-y-4 py-4">
              <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    {profile.linkedIdentity ? (
                      <ProfileLinkedIdentityLink
                        ...
                        ariaLabel={t(`linkedProfileAria.${profile.linkedIdentity.providerId}`)}
                      />
                    ) : null}
                  </div>
                ...
                {profile.bio ? (
                  <p className="text-sm leading-6 text-foreground/85">{profile.bio}</p>
                ) : null}
              </div>
              ...
              {profile.isSelf && profile.publicProfileEnabled && wechatShareEnabled ? (
                <ProfileWechatShareButton
                  shareUrl={
                    buildAbsoluteUrl(`/${locale}/u/${encodeURIComponent(profile.username)}`) ??
              ...
              <ProfileArenaLevelBar locale={locale} score={profile.overview.arenaScore} />
          ...
          <Card className="gap-0 overflow-hidden p-0 shadow-sm ring-1 ring-border/60">
            <header className="flex flex-row flex-wrap items-center justify-between gap-x-3 gap-y-1 border-b border-border/60 bg-muted/40 px-4 py-2.5 dark:bg-muted/25">
              <div className="min-w-0">
                <CardTitle className="text-base leading-tight">{t("activityTitle")}</CardTitle>
              </div>
              {heatmapMarkdown ? (
          ...
          {profile.isSelf && !profile.publicProfileEnabled ? (
            <Card className="border-dashed bg-card ring-1 ring-border/60">
              <CardContent className="space-y-2 py-1">
                <div className="text-sm font-medium">{t("privateBannerTitle")}</div>
                <p className="text-sm text-muted-foreground">{t("privateBannerDescription")}</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-4">
            <Card className="bg-card shadow-sm ring-1 ring-border/60">
              <CardHeader className="pb-1">
                <CardTitle className="text-sm text-muted-foreground">{t("totalTokens")}</CardTitle>
              </CardHeader>
            ...
            <Card className="bg-card shadow-sm ring-1 ring-border/60">
              <CardHeader className="pb-1">
                <CardTitle className="text-sm text-muted-foreground">{t("activeTime")}</CardTitle>
              </CardHeader>
              <CardContent className="pb-1">
                <div className="text-xl font-semibold">
            ...
            <Card className="bg-card shadow-sm ring-1 ring-border/60">
              <CardHeader className="pb-1">
                <CardTitle className="text-sm text-muted-foreground">{t("sessions")}</CardTitle>
              </CardHeader>
              <CardContent className="pb-1">
                <div className="text-xl font-semibold">
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <Card className="gap-0 overflow-hidden p-0 shadow-sm ring-1 ring-border/60">
              <header className="border-b border-border/60 bg-muted/40 px-4 py-2.5 dark:bg-muted/25">
                <div className="min-w-0">
                  <CardTitle className="text-base leading-tight">{t("topTools")}</CardTitle>
                </div>
              </header>
            ...
            <Card className="gap-0 overflow-hidden p-0 shadow-sm ring-1 ring-border/60">
              <header className="border-b border-border/60 bg-muted/40 px-4 py-2.5 dark:bg-muted/25">
                <div className="min-w-0">
                  <CardTitle className="text-base leading-tight">{t("topModels")}</CardTitle>
                </div>
              </header>
              <CardContent className="px-4 pb-4 pt-4">
                <ProfileTopList
